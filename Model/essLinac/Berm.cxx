  ...
  */
{
  ...
  const HeadRule DivHR=getRule("farDivide");
   ELog::EM<<"B divider "<<DivHR.display()<<ELog::endCrit;
  std::string Out;
  Out=ModelSupport::getComposite(SMap,surfIndex," 1 3 5 -6 ") +
    KG.getLinkString(kgSide)+DivHR.complement().display();
  //System.addCell(MonteCarlo::Object(cellIndex++,mat,0.0,Out));
  makeCell("MainShort",System, cellIndex++,mat,0.0,Out);
  addOuterSurf(Out);

  Out=ModelSupport::getComposite(SMap,surfIndex," -2 3 5 -6 ") +
    KG.getLinkString(kgSide)+DivHR.display();
  //System.addCell(MonteCarlo::Object(cellIndex++,mat,0.0,Out));
  makeCell("MainLong",System, cellIndex++,mat,0.0,Out);
  addOuterUnionSurf(Out);
  
  Out=ModelSupport::getComposite(SMap,surfIndex," 1 -2 -4 5 ") +
    KG.getLinkString(-kgSide) + KG.getLinkString(kgFloor);
  makeCell("MainRight",System, cellIndex++,mat,0.0,Out);
  addOuterUnionSurf(Out);
