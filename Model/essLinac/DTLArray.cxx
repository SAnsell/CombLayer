...
DTLArray::DTLArray(const std::string& baseKey,const std::string& Key)  :
  ...
  /*!
    ...
    \param baseKey :: Construction name
...
DTLArray::createLinks()
  ...
  */
{
  ELog::RegMethod RegA("DTLArray","createLinks");

  const int N = std::accumulate
    (dtlSections.begin(),dtlSections.end(),0,
     [](const size_t total,const std::shared_ptr<DTL>& d)
     { 
  ...
  size_t index(0);
  for (const std::shared_ptr<DTL>& dPtr : dtlSections)
    {
      for(size_t j=0; j< dPtr->NConnect(); j++)
	setUSLinkCopy(index++, *dPtr, j);
    }
  return;
}
  ...
  */
{
  ...
  std::string Out;  // why out 
  for (size_t i=0; i<nDTL; i++)
    {
      std::shared_ptr<DTL> dPtr(new DTL(baseName,"DTL",i+1));
      OR.addObject(dPtr);
      if (i==0)
	dPtr->createAll(System,FC,sideIndex);
      else
	dPtr->createAll(System,*dtlSections.back(),2);
      
      dtlSections.push_back(dPtr);
    }

  if (nDTL)
    {
      const std::shared_ptr<DTL>& dPtr=dtlSections.front();
      HeadRule outerHR=dPtr->getFullRule(-1);
      outerHR.addIntersection(dPtr->getFullRule(-3));
      outerHR.addIntersection(dtlSections.back()->getFullRule(-2));
    
      addOuterSurf(outerHR.display());
    }
  createLinks();
