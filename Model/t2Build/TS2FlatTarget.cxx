  ...
  */
{
  ...
  if (surfThick>Geometry::zeroTol && nLayers>1)
    {
      ...
      System.addCell(cellIndex++,wMat,0.0,HR);
  ...
  System.addCell(cellIndex++,taMat,0.0,HR);
  ...
  HR=ModelSupport::getHeadRule(SMap,buildIndex,"2 -101 190");
  System.addCell(cellIndex++,0,0.0,HR);
  ELog::EM<<"Cell == "<<HR<<" ::: "<<cellIndex-1<<ELog::endDiag;

  // FRONT Plate:
  HR=ModelSupport::getHeadRule(SMap,buildIndex,"-1 -27 301");
  System.addCell(cellIndex++,taMat,0.0,HR);

  HR=ModelSupport::getHeadRule(SMap,buildIndex,"-1 -47 311 (27 : -301)");
  System.addCell(cellIndex++,waterMat,0.0,HR);

  HR=ModelSupport::getHeadRule(SMap,buildIndex,"-1 -57 321 (47 : -311)");
  System.addCell(cellIndex++,taMat,0.0,HR);

  HR=ModelSupport::getHeadRule(SMap,buildIndex,"-1 -101 331 (57 : -321)");
  System.addCell(cellIndex++,0,0.0,HR);
  

  // Set EXCLUDE:
  HR=ModelSupport::getHeadRule
    (SMap,buildIndex,"331 -207 (-101 : (201 -202))");
  addOuterSurf(HR);
  return;
}
