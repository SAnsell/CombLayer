...
ConicPipe::ConicPipe(const std::string& Key) : 
...
ConicPipe::ConicPipe(const ConicPipe& A) : 
  ...
  /*!
    Copy constructor
    \param A :: ConicPipe to copy
  */
{}

ConicPipe&
ConicPipe::operator=(const ConicPipe& A)
  /*!
    Assignment operator
    \param A :: ConicPipe to copy
    \return *this
...
void
ConicPipe::populate(const FuncDataBase& Control)
  /*!
    Populate all the variables
    \param Control :: DataBase of variables
  */
{
  ELog::RegMethod RegA("ConicPipe","populate");
  
  FixedOffset::populate(Control);

  // Void + Fe special:
rStart=Control.EvalDefVar<double>(keyName+"RadiusStart",-1.0);
rEnd=Control.EvalDefVar<double>(keyName+"RadiusEnd",-1.0);
/*    
...
void
ConicPipe::createUnitVector(const attachSystem::FixedComp& FC,
  ...
  */
{
  ELog::RegMethod RegA("ConicPipe","createUnitVector");
...
void
ConicPipe::applyActiveFrontBack()
  /*!
   ...
   */
{
  ELog::RegMethod RegA("ConicPipe","applyActiveFrontBack");
...
void
ConicPipe::createSurfaces()
  /*!
    Create the surfaces
  */
{
  ELog::RegMethod RegA("ConicPipe","createSurfaces");
...
double  angle=atan((rStart-rEnd)/length);
 double offset1 = length*rStart/(rEnd-rStart); 
 double offset2 = offset1 + feThick/sin(angle);
 double offset3 = offset1 - claddingThick/sin(angle);
  ModelSupport::buildCone(SMap,buildIndex+17,
			  Origin-Y*offset1-Y*0.5*length,Y,angle*180.0/3.1415);
  
  ModelSupport::buildCone(SMap,buildIndex+7,
			  Origin-Y*offset2-Y*0.5*length,Y,angle*180/3.1415);

  ModelSupport::buildCone(SMap,buildIndex+27,
			  Origin-Y*offset3-Y*0.5*length,Y,angle*180/3.1415);
 ...
 ModelSupport::buildPlane(SMap,buildIndex+111,Origin-Y*0.5*length,Y);
...
void
ConicPipe::createObjects(Simulation& System)
  /*!
    Adds the vacuum box
    \param System :: Simulation to create objects in
   */
{
  ELog::RegMethod RegA("ConicPipe","createObjects");
...
void
ConicPipe::createDivision(Simulation& System)
  /*!
    Divide the vacuum pipe into sections if needed
    \param System :: Simulation
  */
{
  ELog::RegMethod RegA("ConicPipe","createDivision");
  if (nDivision>1)
    {
...
void
ConicPipe::createLinks()
  /*!
    Determines the link point on the outgoing plane.
    It must follow the beamline, but exit at the plane
  */
{
  ELog::RegMethod RegA("ConicPipe","createLinks");
...
void
ConicPipe::setFront(const attachSystem::FixedComp& FC,
		     const long int sideIndex,
		     const bool joinFlag)
  /*!
    Set front surface
   ...
   */
{
  ELog::RegMethod RegA("ConicPipe","setFront");
...
void
ConicPipe::setBack(const attachSystem::FixedComp& FC,
		    const long int sideIndex,
		    const bool joinFlag)
  /*!
    Set Back surface
   ...
   */
{
  ELog::RegMethod RegA("ConicPipe","setBack");
...
void
ConicPipe::createAll(Simulation& System,
  ...
  */
{
  ELog::RegMethod RegA("ConicPipe","createAll(FC)");
