 ...
 * File:   construct/D03SteelSpacing.cxx
 *
...
D03SteelSpacing::D03SteelSpacing(const std::string& Key) : 
...
D03SteelSpacing::D03SteelSpacing(const D03SteelSpacing& A) : 
  ...
  /*!
    Copy constructor
    \param A :: D03SteelSpacing to copy
  */
{}

D03SteelSpacing&
D03SteelSpacing::operator=(const D03SteelSpacing& A)
  /*!
    Assignment operator
    \param A :: D03SteelSpacing to copy
    \return *this
...
D03SteelSpacing::~D03SteelSpacing() 
  /*!
    Destructor
  */
{}
...
void
D03SteelSpacing::removeFrontOverLap()
  /*!
    Remove segments that are completly covered by the
    active front.
  */
{
  ELog::RegMethod RegA("D03SteelSpacing","removeFrontOverLap");
...
void
D03SteelSpacing::populate(const FuncDataBase& Control)
  /*!
    Populate all the variables
    \param Control :: DataBase of variables
  */
{
  ELog::RegMethod RegA("D03SteelSpacing","populate");
  ...
  nWallLayersR=Control.EvalDefVar<size_t>(keyName+"NWallLayersR",1);
  nWallLayersL=Control.EvalDefVar<size_t>(keyName+"NWallLayersL",1);
  ...
  */
  ...
  nRoofLayers=Control.EvalDefVar<size_t>(keyName+"NRoofLayers",1);
...
void
D03SteelSpacing::createUnitVector(const attachSystem::FixedComp& FC,
  ...
  */
{
  ELog::RegMethod RegA("D03SteelSpacing","createUnitVector");
  ...
  */
{
  ...
  for(size_t i=0;i<nWallLayersL;i++)
   ...
   Origin+X*
      ...
      */ WIL+=10;
    }}else{ ModelSupport::buildPlane(SMap,WIL+3, Origin-X*left,X);
       WIL+=10;}
  ...
  (voidRight+(right-voidRight)*(wallFracR[i]-1./nWallLayersR)/(1.-1./nWallLayersR)),X);
      WIR+=10;
    }}else{ ModelSupport::buildPlane(SMap,WIR+4, Origin+X*left,X);
       WIR+=10;}
  ...
  for(size_t i=0;i<nRoofLayers;i++)
    {
	 ...
	 +(height-voidHeight)*(roofFrac[i]-1./nRoofLayers)/(1.-1./nRoofLayers)),Z);
      RI+=10;
    }}else{ ModelSupport::buildPlane(SMap,RI+6, Origin+Z*height,Z);
      RI+=10;}
...
void
D03SteelSpacing::createObjects(Simulation& System)
  /*!
    Adds the vacuum box
    \param System :: Simulation to create objects in
   */
{
  ELog::RegMethod RegA("D03SteelSpacing","createObjects");
  ...
  */
  ...
  for(size_t index=0;index<nSeg;index++)
	...
	//std::cout << "Spacing: making void" << std::endl;
...
void
D03SteelSpacing::createLinks()
  /*!
    Determines the link point on the outgoing plane.
    It must follow the beamline, but exit at the plane
  */
{
  ELog::RegMethod RegA("D03SteelSpacing","createLinks");
...
void
D03SteelSpacing::setFront(const attachSystem::FixedComp& FC,
		     const long int sideIndex)
  /*!
    Set front surface
   ...
   */
{
  ELog::RegMethod RegA("D03SteelSpacing","setFront");
...
void
D03SteelSpacing::setBack(const attachSystem::FixedComp& FC,
		    const long int sideIndex)
  /*!
    Set back surface
    \param FC :: FixedComponent 
   ...
   */
{
  ELog::RegMethod RegA("D03SteelSpacing","setBack");
...
HeadRule
D03SteelSpacing::getXSectionIn() const
  /*!
    Get the line shield inner void section
    \return HeadRule of cross-section
   */
{
  ELog::RegMethod RegA("D03SteelSpacing","getXSectionIn");
...
void
D03SteelSpacing::createAll(Simulation& System,
  ...
  */
{
  ELog::RegMethod RegA("D03SteelSpacing","createAll(FC)");
