 ...
 * File:   construct/LineShieldPlus.cxx
 *
...
LineShieldPlus::LineShieldPlus(const std::string& Key) : 
...
LineShieldPlus::LineShieldPlus(const LineShieldPlus& A) : 
  ...
  /*!
    Copy constructor
    \param A :: LineShieldPlus to copy
  */
{}

LineShieldPlus&
LineShieldPlus::operator=(const LineShieldPlus& A)
  /*!
    Assignment operator
    \param A :: LineShieldPlus to copy
    \return *this
...
void
LineShieldPlus::removeFrontOverLap()
  /*!
    Remove segments that are completly covered by the
    active front.
  */
{
  ELog::RegMethod RegA("LineShieldPlus","removeFrontOverLap");
...
void
LineShieldPlus::populate(const FuncDataBase& Control)
  /*!
    Populate all the variables
    \param Control :: DataBase of variables
  */
{
  ELog::RegMethod RegA("LineShieldPlus","populate");
  ...
  FinalThick=
        Control.EvalPair<double>(keyName+"FinalLayerThick","DefaultTileThick");
 if (FinalThick>0.0){
   /*left+=FinalThick;
 right+=FinalThick;
   */
 ...
 /*
  ...
  wallFrac.pop_back();
  wallFrac.push_back(1.0/nWallLayers*(1.+
       ((nWallLayers-1.)*(left-FinalThick)-voidLeft*(nWallLayers-1.))/(left-voidLeft)));
  wallFrac.push_back(1.0);
 
 for (int i=1;i<nWallLayers-2;i++)
    {
      // wallFrac[i]*=(nWallLayers*1.0)/(nWallLayers-1.0);
	   // std::cout << wallFrac[i] <<" " ;
  wallFrac[i]=wallFrac[0]+i*(wallFrac[nWallLayers-1]-wallFrac[0])/(nWallLayers-2.);
    };
 */
...
void
LineShieldPlus::createUnitVector(const attachSystem::FixedComp& FC,
  ...
  */
{
  ELog::RegMethod RegA("LineShieldPlus","createUnitVector");
  ...
  */
{
  ...
  int WI(buildIndex);
  size_t imax(nWallLayers);
  if (FinalThick>0.0) imax--;
  for(size_t i=0;i<imax;i++)
    {
      ModelSupport::buildPlane(SMap,WI+3,
   Origin-X*
	(voidLeft+(left-voidLeft)*(wallFrac[i]-1./imax)/(1.-1./imax)),X);
      ModelSupport::buildPlane(SMap,WI+4,
   Origin+X*
     (voidRight+(right-voidRight)*(wallFrac[i]-1./imax)/(1.-1./imax)),X);
      WI+=10;
    }
  if(FinalThick>0.0){ // Extra planes if final B4C layer is requested
      ModelSupport::buildPlane(SMap,WI+3,
   Origin-X*(left+FinalThick),X);
      ModelSupport::buildPlane(SMap,WI+4,
   Origin+X*(right+FinalThick),X);
  }
  ...
  for(size_t i=0;i<nFloorLayers;i++)
    {
     ...
     Origin-Z*(voidDepth
           +(depth-voidDepth)*(floorFrac[i]-1./nFloorLayers)/(1.-1./nFloorLayers)),Z);
			       //
			       //		 voidDepth*(1-1./nWallLayers)
			       //+(depth-voidDepth*(1-1./nWallLayers))*floorFrac[i]),Z);
      FI+=10;
    }  
  return;
}
...
void
LineShieldPlus::createObjects(Simulation& System)
  /*!
    Adds the vacuum box
    \param System :: Simulation to create objects in
   */
{
  ELog::RegMethod RegA("LineShieldPlus","createObjects");
...
void
LineShieldPlus::createLinks()
  /*!
    Determines the link point on the outgoing plane.
    It must follow the beamline, but exit at the plane
  */
{
  ELog::RegMethod RegA("LineShieldPlus","createLinks");
...
void
LineShieldPlus::setFront(const attachSystem::FixedComp& FC,
		     const long int sideIndex)
  /*!
    Set front surface
   ...
   */
{
  ELog::RegMethod RegA("LineShieldPlus","setFront");
...
void
LineShieldPlus::setBack(const attachSystem::FixedComp& FC,
		    const long int sideIndex)
  /*!
    Set back surface
    \param FC :: FixedComponent 
   ...
   */
{
  ELog::RegMethod RegA("LineShieldPlus","setBack");
...
HeadRule
LineShieldPlus::getXSectionIn() const
  /*!
    Get the line shield inner void section
    \return HeadRule of cross-section
   */
{
  ELog::RegMethod RegA("LineShieldPlus","getXSectionIn");
...
void
LineShieldPlus::createAll(Simulation& System,
  ...
  */
{
  ELog::RegMethod RegA("LineShieldPlus","createAll(FC)");
