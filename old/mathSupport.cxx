...
mathFunc::binSearch(const typename std::vector<T>::const_iterator& pVecB,
		    ...
		    const T& V)
  /*!
    Determine a binary search of a componenet
  ...
  */
{
  ...
  if (cf>=0)          /* Real Roots */
    {
      q=(b>=0) ? -0.5*(b+sqrt(cf)) : -0.5*(b-sqrt(cf));
  ...
  /* Complex roots */
  Ans[0].re=-0.5*b/a;
  Ans[0].im=(b>=0) ? -0.5*sqrt(-cf)/a : 0.5*sqrt(-cf)/a; 
  q=b*b-cf;
  Ans[1].re=(-0.5*b*c)/q;
  Ans[1].im=(b>=0.0) ? 0.5*c*sqrt(-cf) : -0.5*c*sqrt(-cf);
  OutAns.first=std::complex<double>(Ans[0].re,Ans[0].im);
  OutAns.second=std::complex<double>(Ans[1].re,Ans[1].im);
  return 2;
}
...

{
  ...
  if (discrim<1e-13) /* All roots real, at least two are equal. */
    {
...
mathFunc::crossSort(std::vector<T>& pVec,std::vector<U>& Base)
  /*!
    Function to take a vector (pVec) and sort the vector, but at the
    same time apply the same transform to anyother reciprical vector (Base).
    T must have less_than operator
    \param pVec :: Vector of variables to index
    \param Base :: Other vector to keep in order (not used for sorting)
  */
{
...
template void mathFunc::crossSort(std::vector<int>&,std::vector<const Surface*>&);
