...
template<typename T> 
void
Matrix<T>::idenityMatrix()
  /*!
    Makes the matrix an idenity matrix
    Does nothing if the matrix is not square
  */
{
  if (nx*ny)
    {
  ...
  */
{
  ...
  for(int i=0;i<100;i++)        //max 50 iterations
	...
	for(int iq=ip+1;iq<nx;iq++)
	  sm+=fabs(A.V[ip][iq]);
//      std::cerr<<"Iterations :"<<iteration<<" "<<sm<<std::endl;
      if (static_cast<float>(sm)==0.0)           // Nothing to do return...
	{
	  // Make OUTPUT -- D + A
	  // sort Output::
//	  std::vector<int> index;
...
//	      EigenVector
	  ...
	  for(int iq=ip+1;iq<nx;iq++)
	    {
	      ...
	      if (i>4 && static_cast<float>(fabs(Diag[ip]+g))==static_cast<float>(fabs(Diag[ip]))
  ...
  for(int ix=0;ix<nx;ix++)
    DiagMatrix.V[ix][ix]=static_cast<T>(Diag[ix]);
  std::cerr<<"Error :: Iterations are a problem:"<<sm<<std::endl;
  return 0;
}
