...
*/
{
  ...
  /*
    +1 to account for start/end half-folds,
     i.e. single fold bellow has 4 halfFolds: \/\/
     \/\/\/  \/\/\/\/ -> 1:2, 2:3 3:4
  */
  const double foldLength = L/(nFolds+1);
...
Bellows::getBellowThick() const
  ...
  */
{
  ELog::RegMethod RegA("Bellows","getBellowThick");
  
  const double halfFold = getHalfFoldLength();
  const double R = std::max(flangeA.radius, flangeB.radius);
  /*
    bellow outer radius at max compression TODO: don't guess, make it a variable, but check outerVoid below
  */
  ...
  if (maxThick<halfFold+Geometry::zeroTol)
    throw ColErr::NumericalAbort
      (keyName + ": impossible combination of R="+std::to_string(R)+
       ", length="+std::to_string(getBellowLength())+
       " and nFolds="+std::to_string(nFolds)+". Try to increase nFolds.");

  return std::sqrt(maxThick*maxThick - halfFold*halfFold);
}
  ...
  */
{
  ...
  engActive=Control.EvalDefPair<int>(keyName+"EngineeringActive",
				     "EngineeringActive",0);
  ...
  if (engActive)
    {
      ...
      ModelSupport::buildShiftedPlane(SMap,buildIndex+122,
				      buildIndex+121,Y,wallThick);
      ModelSupport::buildShiftedPlane(SMap,buildIndex+222,
				      buildIndex+221,Y,-wallThick);

      const double halfFold = getHalfFoldLength();
      const double angle = std::atan(bellowThick/halfFold)*180.0/M_PI;
      const double rt = halfFold*getBellowRadius()/getBellowThick();
      const double bl2 = getBellowLength()/2.0;
      const double y0 = -bl2 + rt;
      const double y1 = -bl2 - rt + 2.0*halfFold;
      const double dWall = wallThick/std::sin(angle*M_PI/180.0)/2.0;
  ...
  */
{
  ...
  const HeadRule& voidFront =
    (flangeA.radius>flangeA.radius+Geometry::zeroTol) ?
    HeadRule(SMap,buildIndex,101) : frontHR;
  const HeadRule& voidBack =
    (flangeB.radius>flangeB.radius+Geometry::zeroTol) ?
