...
sub new
{
  ...
  my $self={
    ...
    fflag => "-fPIC -W -Wall -Wextra -Wno-comment -fexceptions ",
    ...
    fortDir => undef,          ## Array of all our fortran directories
...
sub hasFORTFiles
  ##
  ## Simple test to find if we have .f files
  ## files in the directory 
{
  
  my $Dname=shift;  ## Directory to search
  
  my $fortAll=`ls ./$Dname/*.f 2> /dev/null`;
  return ($fortAll) ? 1 : 0;
}
  ...
  foreach my $dname (@DPts)
    {
	  ...
	  if (!$flag)
	    {
	      push(@subDir,$1) if (hasFORTFiles($dname));
	    }
	}
    }

  $self->addFortDir($tName,\@subDir) if (@subDir);
  return;
}
...
# print $DX "set(CMAKE_CXX_CLANG_TIDY clang-tidy; -checks=*)\n";
  ...
  if (%{$self->{fortDir}})
    {
      print $DX "enable_language(Fortran)\n";
      print $DX "set(CMAKE_Fortran_COMPILER ",$self->{fcomp},")\n";
      print $DX "set(CMAKE_Fortran_FLAGS \"",
	  $self->{fflag}.$self->{optimise}.$self->{debug},"\")\n";
      print $DX "set(CMAKE_Fortran_RELEASE_FLAGS \"",
	  $self->{fflag}." -O2 ".$self->{debug},"\")\n";
  ...
  # Write out the include list
{
  ...
  foreach my $item (keys (%{$self->{fortDir}}))
  {
      my $val=$self->{fortDir}{$item};
